...
export async function anchorAuditHead(organizationId:string,actorUserId?:string|null){
 ...
 const record={organizationId,projectId:`audit:${organizationId}`,assetId:`audit:${organizationId}`,recordId:`audit-root:${event.id}`,type:'AUDIT_ROOT',evidenceHash:integrity.head,timestamp:new Date(event.created_at).toISOString(),signer:'STRATUM_AUDIT_ROOT',payloadHash:integrity.head};
