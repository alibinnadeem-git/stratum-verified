...
export async function telemetrySchemaReady(){
  const r=await query<{ready:boolean}>(`SELECT to_regclass('public.telemetry_sources') IS NOT NULL AND to_regclass('public.telemetry_point_bindings') IS NOT NULL AND to_regclass('public.operational_live_iterations') IS NOT NULL AS ready`);
...
function statusClassFromNumber(raw:number):TelemetryQuality{
  const code=(raw>>>0);
  const severity=code&0xC0000000;
  if(severity===0x80000000)return'BAD';
  if(severity===0x40000000)return'UNCERTAIN';
  return'GOOD';
}
...
export async function ingestTelemetryBatch(input:{organizationId:string;sourceCode:string;samples:RawTelemetrySample[]}){
  ...
  const result=await tx(async client=>{
    ...
    for(const item of prepared){
      ...
      const sourceRef={...(b.source_ref&&typeof b.source_ref==='object'&&!Array.isArray(b.source_ref)?b.source_ref as Record<string,unknown>:{}),adapterType:source.adapter_type,sourceCode:source.source_code,externalPointKey:item.sample.externalPointKey,bindingId:b.id,truthBoundary:TELEMETRY_TRUTH_BOUNDARY};
      const metadata={rawQuality:item.sample.quality??null,mappingEvidence:b.mapping_evidence,authority:TELEMETRY_AUTHORITY,controlAuthority:false,verifiedBaselineMutation:false};
      ...
      if(!inserted.rows[0]){
        ...
        duplicates.push({externalPointKey:item.sample.externalPointKey,semanticKey:b.semantic_key,observationId:existing.rows[0]?.id||null,idempotencyKey:item.idempotencyKey});
        continue;
      }
      ...
      accepted.push({externalPointKey:item.sample.externalPointKey,semanticKey:b.semantic_key,assetId:b.asset_id,touchpointId:b.touchpoint_id,observationId,quality:item.quality,confidence:item.confidence,engineeringUnit:b.engineering_unit,idempotencyKey:item.idempotencyKey});
    }
