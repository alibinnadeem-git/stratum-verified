...
export type StoredSigner={userId:string;keyId:string;privateKey:CryptoKey;publicKey:CryptoKey;fingerprint:string;createdAt:string};
...
export async function getLocalSigner(userId:string){const db=await openDb();return new Promise<StoredSigner|undefined>((resolve,reject)=>{const tx=db.transaction(STORE,'readonly');const r=tx.objectStore(STORE).get(userId);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)})}
...
export async function getOrCreateSigner(userId:string){
 const current=await getLocalSigner(userId);if(current)return current;
...
export async function revokeAndRotateSigner(userId:string){const current=await getLocalSigner(userId);if(current){const r=await fetch('/api/security/signing-keys',{method:'DELETE',headers:{'content-type':'application/json'},body:JSON.stringify({keyId:current.keyId})});if(!r.ok){const j=await r.json().catch(()=>({}));throw new Error(j.error||'Could not revoke current signing key')}}await removeLocalSigner(userId);return getOrCreateSigner(userId)}
