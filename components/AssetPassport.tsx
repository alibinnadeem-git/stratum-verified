...
export default function AssetPassport({asset}:{asset:Asset}){return <div className="passport card">
 ...
 <div className="passport-grid"><div className="passport-facts"><div><span>Asset ID</span><strong>{asset.id}</strong></div><div><span>Serial Number</span><strong>{asset.serial}</strong></div><div><span>System</span><strong>{asset.system}</strong></div><div><span>Location</span><strong>{asset.location}</strong></div><div><span>Status</span><strong>{asset.status}</strong></div><div><span>Latest DIR</span><strong>{asset.block||'Pending'}</strong></div></div><div className="qr-wrap"><AssetQR value={asset.qrToken}/><small>Scan asset passport</small></div></div>
