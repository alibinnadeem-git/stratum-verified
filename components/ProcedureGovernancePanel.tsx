...
export default function ProcedureGovernancePanel({procedures}:{procedures:Procedure[]}){
 ...
 return <section className="card"><div className="section-head"><div><div className="eyebrow">STRATUM Procedure Governance</div><h2>Review, approve and freeze executable procedures</h2><p className="muted">Approval is restricted to project-management or organization-administration roles. Once approved, procedure steps are immutable; changes require a new version.</p></div><span className="badge">SEPARATION OF DUTIES</span></div>
  ...
  {selected&&<div className="button-row">{selected.status==='DRAFT'&&<button className="action secondary" disabled={!!busy} onClick={()=>setStatus('REVIEW')}>{busy==='REVIEW'?'Updating…':'Submit for Review'}</button>}{selected.status==='REVIEW'&&<button className="action" disabled={!!busy||selected.stepCount<1} onClick={()=>setStatus('APPROVED')}>{busy==='APPROVED'?'Approving…':'Approve & Freeze Procedure'}</button>}{selected.status==='APPROVED'&&<button className="action secondary" disabled={!!busy} onClick={()=>setStatus('RETIRED')}>{busy==='RETIRED'?'Retiring…':'Retire Procedure'}</button>}{selected.status==='RETIRED'&&<span className="muted">Retired procedure is immutable. Create a new version to make changes.</span>}</div>}
 </section>
}
