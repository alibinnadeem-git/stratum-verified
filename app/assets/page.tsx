...
export default async function Assets(){
 const session=await requirePageSession('/assets');
 const projectIds=await accessibleProjectIds(session);
 const assets=await liveAssets(session.organizationId,projectIds);
 return <><div className="page-head"><div><div className="eyebrow">Asset Registry · Live</div><h1 className="title">Asset passports</h1><p className="subtitle">Durable equipment identities backed by the live STRATUM Verified database and finalized Digital Immutable Records.</p></div><Link className="action" href="/workflows">+ Create / verify asset</Link></div>
 ...
 {!assets.length&&<tr><td colSpan={6}><div className="muted">No assets are assigned to your accessible projects.</div></td></tr>}</tbody></table></div></>;
}
