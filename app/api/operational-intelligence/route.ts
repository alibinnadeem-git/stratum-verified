...
const Procedure=z.object({kind:z.literal('procedure'),projectId:z.string().uuid(),procedureCode:z.string().min(2).max(100),procedureType:z.enum(['SOP','MOP','EOP','MAINTENANCE','LOTO','COMMISSIONING']),title:z.string().min(2).max(220),purpose:z.string().max(2000).optional(),version:z.string().min(1).max(30).default('1.0'),status:z.enum(['DRAFT','REVIEW','APPROVED']).optional(),sourceRefs:z.array(z.record(z.string(),z.unknown())).default([])});
...
export async function POST(req:Request){
 try{
  ...
  if(body.kind==='procedure'){
   ...
   const r=await query<any>(`INSERT INTO operational_procedures(organization_id,project_id,procedure_code,procedure_type,title,purpose,version,status,source_refs,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,'DRAFT',$8,$9) RETURNING *`,[session.organizationId,body.projectId,body.procedureCode,body.procedureType,body.title,body.purpose||null,body.version,JSON.stringify(body.sourceRefs),session.userId]);
   await appendAudit({organizationId:session.organizationId,actorUserId:session.userId,action:'OPERATIONAL_PROCEDURE_CREATE',entityType:body.procedureType,entityId:r.rows[0].id,metadata:{projectId:body.projectId,procedureCode:body.procedureCode,version:body.version,status:'DRAFT',requestedStatus,effectiveRole}});return NextResponse.json({...r.rows[0],requestedStatusNormalizedTo:'DRAFT'},{status:201});
  }
  if(body.kind==='procedure_status'){
   const p=await query<any>(`SELECT id,status,created_by::text,(SELECT count(*)::int FROM operational_procedure_steps s WHERE s.procedure_id=operational_procedures.id AND s.organization_id=operational_procedures.organization_id) step_count FROM operational_procedures WHERE id=$1 AND organization_id=$2 AND project_id=$3 LIMIT 1`,[body.procedureId,session.organizationId,body.projectId]);if(!p.rows[0])return NextResponse.json({error:'Procedure is not available in the selected project.'},{status:404});
   const current=String(p.rows[0].status);const next=body.status;
   const validTransition=(current==='DRAFT'&&next==='REVIEW')||(current==='REVIEW'&&next==='APPROVED')||(current==='APPROVED'&&next==='RETIRED');
   if(!validTransition)return NextResponse.json({error:`Invalid procedure transition ${current} → ${next}. Required sequence is DRAFT → REVIEW → APPROVED → RETIRED.`},{status:409});
   if(next==='APPROVED'||next==='RETIRED'){
   ...
   if(next==='APPROVED'&&p.rows[0].step_count<1)return NextResponse.json({error:'A procedure must contain at least one machine-readable step before approval.'},{status:409});
   if(next==='APPROVED'&&p.rows[0].created_by===session.userId)return NextResponse.json({error:'Independent approval required: the procedure author cannot approve the same executable procedure version.'},{status:409});
   const r=await query<any>(`UPDATE operational_procedures SET status=$1,approved_by=CASE WHEN $1='APPROVED' THEN $2 ELSE approved_by END,approved_at=CASE WHEN $1='APPROVED' THEN now() ELSE approved_at END,updated_at=now() WHERE id=$3 AND organization_id=$4 AND project_id=$5 RETURNING *`,[next,session.userId,body.procedureId,session.organizationId,body.projectId]);
   await appendAudit({organizationId:session.organizationId,actorUserId:session.userId,action:`OPERATIONAL_PROCEDURE_${next}`,entityType:'PROCEDURE',entityId:body.procedureId,metadata:{projectId:body.projectId,previousStatus:current,newStatus:next,effectiveRole,independentApproval:next==='APPROVED'}});return NextResponse.json(r.rows[0]);
  }
  if(body.kind==='step'){
