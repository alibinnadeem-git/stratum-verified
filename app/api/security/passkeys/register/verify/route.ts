...
export async function POST(req:Request){try{
 ...
 const out=await tx(async c=>{const r=await c.query(`INSERT INTO webauthn_credentials(organization_id,user_id,credential_id,public_key,counter,transports,device_type,backed_up,label) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT(credential_id) DO UPDATE SET revoked_at=NULL,revoked_by=NULL,last_used_at=now() RETURNING id,credential_id,label,device_type,backed_up,created_at`,[s.organizationId,s.userId,credential.id,Buffer.from(credential.publicKey),credential.counter||0,credential.transports||[],info.credentialDeviceType||null,!!info.credentialBackedUp,body.label||'Passkey']);await c.query(`UPDATE webauthn_challenges SET consumed_at=now() WHERE id=$1`,[challenge.id]);await c.query(`INSERT INTO security_events(organization_id,user_id,actor_user_id,event_type,metadata) VALUES($1,$2,$2,'PASSKEY_REGISTERED',$3::jsonb)`,[s.organizationId,s.userId,JSON.stringify({credentialId:credential.id,label:body.label||'Passkey'})]);return r.rows[0]});
