...
async function assetInProject(organizationId:string,projectId:string,assetId:string){const r=await query(`SELECT 1 FROM assets WHERE id=$1 AND organization_id=$2 AND project_id=$3`,[assetId,organizationId,projectId]);return !!r.rowCount}
...
export async function POST(req:Request){
 try{
  ...
  if(body.kind==='binding'){
   const source=await query<any>(`SELECT id::text,status FROM telemetry_sources WHERE id=$1 AND organization_id=$2 AND project_id=$3 LIMIT 1`,[body.sourceId,session.organizationId,body.projectId]);
   if(!source.rows[0])return NextResponse.json({error:'Telemetry source is not available in the selected project.'},{status:404});
   if(!(await assetInProject(session.organizationId,body.projectId,body.assetId)))return NextResponse.json({error:'Binding asset is not available in the selected project.'},{status:403});
   if(body.touchpointId){const t=await query<any>(`SELECT asset_id::text FROM operational_touchpoints WHERE id=$1 AND organization_id=$2 AND project_id=$3 LIMIT 1`,[body.touchpointId,session.organizationId,body.projectId]);if(!t.rows[0])return NextResponse.json({error:'Human Touchpoint is not available in the selected project.'},{status:403});if(t.rows[0].asset_id!==body.assetId)return NextResponse.json({error:'Human Touchpoint must belong to the same STRATUM Asset as the telemetry binding.'},{status:409});}
