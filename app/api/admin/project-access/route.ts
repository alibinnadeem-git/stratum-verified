...
const ProjectRole=z.enum(['PROJECT_MANAGER','TECHNICIAN','INSPECTOR','CLIENT','VIEWER']);
const Change=z.object({projectId:z.string().uuid(),userId:z.string().uuid(),assigned:z.boolean(),projectRole:ProjectRole.nullable().optional()});
...
export async function POST(req:Request){try{const s=await requireSession(['SUPER_ADMIN','ORG_ADMIN']);const b=Change.parse(await req.json());const valid=await query(`SELECT m.role FROM projects p JOIN memberships m ON m.organization_id=p.organization_id AND m.user_id=$2 WHERE p.id=$1 AND p.organization_id=$3`,[b.projectId,b.userId,s.organizationId]);if(!valid.rowCount)return NextResponse.json({error:'Project or user is outside this organization'},{status:404});const orgRole=valid.rows[0].role;if(orgRole==='SUPER_ADMIN'||orgRole==='ORG_ADMIN')return NextResponse.json({error:'Organization administrators are globally scoped and do not need project assignments.'},{status:409});await tx(async c=>{if(b.assigned)await c.query(`INSERT INTO project_memberships(organization_id,project_id,user_id,project_role,created_by) VALUES($1,$2,$3,$4,$5) ON CONFLICT(project_id,user_id) DO UPDATE SET project_role=EXCLUDED.project_role`,[s.organizationId,b.projectId,b.userId,b.projectRole||null,s.userId]);else await c.query(`DELETE FROM project_memberships WHERE organization_id=$1 AND project_id=$2 AND user_id=$3`,[s.organizationId,b.projectId,b.userId])});await appendAudit({organizationId:s.organizationId,actorUserId:s.userId,action:b.assigned?'PROJECT_ACCESS_GRANT':'PROJECT_ACCESS_REVOKE',entityType:'PROJECT_MEMBERSHIP',entityId:`${b.projectId}:${b.userId}`,metadata:{projectId:b.projectId,userId:b.userId,projectRole:b.projectRole||null}});return NextResponse.json({ok:true})}catch(e:any){return NextResponse.json({error:e.message},{status:e.status||400})}}
