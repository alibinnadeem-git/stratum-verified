...
export default async function TwinPage(){
  const session=await requirePageSession('/twin');
  const projectIds=await accessibleProjectIds(session);
  const rows=await liveAssets(session.organizationId,projectIds);
  ...
  return <>
    <div className="page-head"><div><div className="eyebrow">STRATUM Twin · Design → Build → Verify → Operate</div><h1 className="title">The verifiable digital twin of physical infrastructure.</h1><p className="subtitle">Submit a GLB/GLTF design, inspect its geometry, bind meaningful model objects to durable STRATUM Verified asset identities and connect approved lifecycle proof to Digital Immutable Records. Model files and private evidence remain off-ledger; only cryptographic proof crosses the trust boundary.</p></div><div className="badge">● DIR trust layer · hashes only</div></div>
    <TwinWorkspace assets={assets} referenceModelUrl={process.env.NEXT_PUBLIC_STRATUM_TWIN_REFERENCE_MODEL_URL}/>
    <div className="card" style={{marginTop:16}}><div className="eyebrow">Trust architecture</div><div className="provenance-map"><div className="prov-step active"><i>L1</i><b>Physical Infrastructure</b><span>QR · NFC · Serial · Installed equipment</span></div><em>→</em><div className="prov-step active"><i>L2</i><b>STRATUM Twin</b><span>Spatial model · systems · asset relationships</span></div><em>→</em><div className="prov-step active"><i>L3</i><b>STRATUM Verified</b><span>Asset identity · evidence · lifecycle records</span></div><em>→</em><div className="prov-step active"><i>L4</i><b>Ledger Adapter</b><span>Canonical hashes only</span></div><em>→</em><div className="prov-step active"><i>L5</i><b>DIR Network</b><span>Immutable proof · validator consensus</span></div></div></div>
  </>;
}
