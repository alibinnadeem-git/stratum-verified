...
Existing application/chain variables remain required. Add:
...
1. Confirm application CI is green and permanent Block 1 / Block 2 regression proofs still pass.
...
10. Verify the resulting chain record in Asset Passport / public verification.
11. Verify the audit hash chain and manually anchor its head once.
12. Confirm the daily audit-root cron sees the already-anchored head and does not create a duplicate.
...
- Administrative audit events are hash-chained. The current verified audit head can be anchored to STRATUM Chain manually or through the daily cron.
